...
const cargarFacturas = async () => {
  try {
    ...
    if (intervalo === "personalizado") {
      if (fechaInicio && fechaFin) {
        ...
        mostrarNotificacion("error", "Seleccione un rango válido de fechas.");
        return;
      }
    } else {
      const hoy = new Date();
      switch (intervalo) {
        ...
        default:
          mostrarNotificacion("error", "Intervalo no reconocido.");
          return;
      }
    }
    ...
    mostrarNotificacion("error", "Error al cargar las facturas.");
  }
};
  ...
  const exportarExcel = () => {
    if (!facturaSeleccionada) {
      mostrarNotificacion("error", "Debe seleccionar una factura para exportar.");
      return;
    }
    ...
    try {
      ...
      mostrarNotificacion("error", "Error al exportar a Excel.");
    }
  };
