...
function Recepciones() {
  ...
  const confirmarSolicitud = () => {
    ...
    if (!isNaN(idSolicitud)) {
      ...
      toast.error("Por favor ingresa un número de solicitud válido.");
    }
  };
  ...
  const abrirModalDetalles = async () => {
    if (!recepcionSeleccionada) {
      ...
      toast.error("Por favor selecciona una recepción válida.");
      return;
    }
    ...
    if (!idSolicitud) {
      ...
      toast.error("Esta recepción no tiene una solicitud de compra asociada.");
      return;
    }
    try {
      ...
      toast.error("Hubo un problema al cargar los detalles de la recepción.");
    }
  };
  ...
  const abrirModalConfirmacion = () => {
    if (!nuevaRecepcion.productos || nuevaRecepcion.productos.length === 0) {
      toast.error("No hay productos en la recepción para confirmar.");
      return;
    }
  ...
  const manejarConfirmacion = async () => {
    ...
    try {
      ...
      if (!fichaDeCosto) {
        ...
        toast.error(`No se pudo obtener la ficha de costo para el producto ${producto.nombre_producto}.`);
        return;
      }
      const nuevoProducto = {
      ...
      toast.error("Ocurrió un error al guardar el producto. Por favor, inténtalo de nuevo.");
    }
  };
  ...
  const crearRecepcionFinal = async () => {
    try {
      ...
      if (recepcionExistente) {
        toast.error(`Ya existe una recepción para la solicitud de compra #${nuevaRecepcion.id_solicitud_compra}.`);
        return;
      }
      ...
      toast.error("Hubo un error al guardar la recepción. Intenta nuevamente.");
    }
  };
  ...
  const seleccionarSolicitud = async (idSolicitud) => {
    ...
    if (!solicitud) {
      ...
      toast.error("Por favor selecciona una solicitud válida.");
      return;
    }
    if (!solicitud.id_proveedor) {
      console.error("La solicitud seleccionada no tiene un proveedor válido.");
      toast.error("La solicitud seleccionada no tiene un proveedor válido.");
      return;
    }
    ...
    try {
      ...
      if (!productosTransformados.length) {
        toast.error("La solicitud no tiene productos válidos asociados.");
        return;
      }
      ...
      toast.error("Hubo un problema al cargar los productos asociados a la solicitud.");
    }
  };
  ...
  const guardarProductoEditado = async () => {
    try {
      ...
      toast.error("Hubo un error al actualizar el producto. Por favor, inténtalo nuevamente.");
    }
  };
  ...
  const guardarRecepcion = async () => {
    ...
    if (!nuevaRecepcion.id_proveedor) {
      toast.error("Selecciona un proveedor válido.");
      return;
    }
    ...
    if (!nuevaRecepcion.fecha_recepcion) {
      toast.error("Selecciona una fecha de recepción.");
      return;
    }
    if (!nuevaRecepcion.id_solicitud_compra) {
      toast.error("Selecciona una solicitud de compra.");
      return;
    }
    try {
      ...
      toast.error("Ocurrió un error al comprobar la recepción. Intenta nuevamente.");
    }
  };
