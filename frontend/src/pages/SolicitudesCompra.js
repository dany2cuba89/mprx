...
// Función para renderizar botones condicionales en el modal de editar solicitud
const renderBotonesModal = () => {
  ...
  return (
    <>
      <button className="add-button" onClick={guardarEdicion}>
        Guardar Cambios
      </button>
      <button className="cancel-button" onClick={cerrarModal}>
        Cancelar
...
const guardarProducto = () => {
  ...
  const cargarSolicitudes = async () => {
    try {
      ...
      mostrarNotificacion("error", "Error al cargar las solicitudes.");
    }
  };
  const cargarProveedores = async () => {
    try {
      ...
      mostrarNotificacion("error", "Error al cargar proveedores.");
    }
  };
...
const calcularRangoPaginas = () => {
  ...
  const handleDeleteConfirmation = async () => {
    if (selectedSolicitud) {
      try {
        ...
        mostrarNotificacion("success", "Solicitud eliminada correctamente.");
        cargarSolicitudes();
        setSelectedSolicitud(null);
      } catch (err) {
        console.error("Error al eliminar solicitud:", err.message);
        mostrarNotificacion("error", "Error al eliminar la solicitud.");
      }
    }
  ...
  const openDeleteConfirmModal = () => {
    if (!selectedSolicitud) {
      mostrarNotificacion("error", "Selecciona una solicitud para eliminar.");
    } else {
  ...
  if (!proveedor) {
    mostrarNotificacion("error", "Proveedor seleccionado no válido.");
    return;
  }
  ...
  try {
    ...
    mostrarNotificacion("success", "Solicitud y productos agregados exitosamente.");
    cerrarModal();
    cargarSolicitudes();
  } catch (err) {
...
const guardarEdicion = async () => {
  ...
  try {
    ...
    if (solicitudActual.estado_solicitud === "Aprobada") {
      ...
      mostrarNotificacion("success", "Solicitud y productos actualizados exitosamente.");
      setEstadoGuardado(false); // Asegurar que el estado vuelva a ser falso
      cerrarModal();
    }
  } catch (err) {
    console.error("Error en guardarEdicion:", err.message);
    mostrarNotificacion("error", `Error al guardar los cambios: ${err.message}`);
  }
};
...
const abrirModalFicha = async () => {
  try {
    ...
    mostrarNotificacion("error", "Error al cargar fichas de costo.");
  }
};
  ...
  const guardarFichaDeCosto = async () => {
    try {
      ...
      mostrarNotificacion("success", "Ficha guardada exitosamente.");
    } catch (error) {
      console.error("Error guardando ficha:", error);
      mostrarNotificacion("error", "Error al guardar la ficha.");
    }
  };
...
<Modal abierto={modalFichaAbierto} cerrarModal={() => setModalFichaAbierto(false)}>
  ...
  <div className="ficha-costo-container">
    ...
    {/* Botones */}
    {/* Botones del modal de ficha de costo */}
<div className="modal-buttons">
  <button
